...
import { createPost, getCities, getPosts } from '../lib/posts'
		...
		try {
			...
			if (
				...
				if (geolocationError.code === 1) {
					// PERMISSION_DENIED
					setLocationError(
						'Location permission denied. Please enable location access to find your city.',
					)
				...
				if (errorMessage.includes('GOOGLE_MAPS_API_KEY')) {
					setLocationError(
						'Location service not configured. Please contact support.',
					)
				} else if (errorMessage.includes('Geocoding failed')) {
					setLocationError(
						'Could not determine your city from GPS coordinates. Please try again.',
					)
				} else {
					...
					<>
						...
						<section className="space-y-8">
							<h2 className="font-[family-name:var(--font-display)] text-2xl font-semibold text-foreground">
								{viewCity
									? `Yips from ${formatCityName(viewCity)}`
									: 'Latest Yips'}
							</h2>
							...
							{posts.length === 0 && (
								<p className="text-center text-muted-foreground">
									{viewCity
										? `No yips from ${formatCityName(viewCity)} yet.`
