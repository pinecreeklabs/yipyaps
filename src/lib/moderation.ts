...
export async function moderateContent(content: string, aiBinding: Ai | undefined): Promise<ModerationResult> {
    ...
    try {
        ...
        console.error('[moderation] Error:', error instanceof Error ? error.message : error)
        return { allowed: false, reason: 'Moderation service unavailable' }
    }
}

export async function logPostEval({ db, postId, result }: LogEvalParams): Promise<void> {
    try {
