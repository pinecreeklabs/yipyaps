...
export const createPost = createServerFn({ method: 'POST' })
    ...
    .handler(async ({ data }): Promise<CreatePostResult> => {
        ...
        const contentPreview = trimmedContent.length > 50 ? `${trimmedContent.substring(0, 50)}...` : trimmedContent

        // Compute S2 cell ID from coordinates
        const cellId = coordsToCellId(latitude, longitude)

        // Resolve city name for display
        const { city } = await resolveCity(latitude, longitude, env.GOOGLE_MAPS_API_KEY)
        ...
        try {
            ...
            return {
                success: false,
                blocked: true,
                message: 'Your post was not published. Please keep it friendly and try again.',
            }
        } catch (error) {
