      ...
      <DialogContent showCloseButton={false} className="text-center">
        <DialogHeader className="items-center">
          <DialogTitle className="font-[family-name:var(--font-display)] text-3xl">
            {state === 'found' ? `You're in ${cityName}!` : 'Welcome to Yipyaps'}
          </DialogTitle>
          <DialogDescription className="text-base">
            {state === 'initial' && 'Share quick notes with your city. Let\'s find where you are.'}
            {state === 'locating' && 'Finding your location...'}
            {state === 'found' && 'Ready to see what your neighbors are sharing?'}
            {state === 'error' && error}
          </DialogDescription>
        </DialogHeader>
